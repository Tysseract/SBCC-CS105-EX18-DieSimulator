...
public class DieSimulatorTester {
	...
	public void dontRollAZero() throws Exception {
		...
		for (int i = 0; i < 100; i++) {
			assertFalse("A die goes from 1 to 6. It should never be zero.", ds.rollTheDie() == 0);
		}
		totalScore += 3;
	}
	...
	public void rollInRange() throws Exception {
		...
		assertTrue("Die must return a number between 1 and 6 inclusive", dieRolls.get(0) == 0);

		for (int j = 1; j < 7; j++) {
			assertTrue("Die must return a number between 1 and 6 inclusive", dieRolls.get(j) > 0);
		}
		totalScore += 4;
	}
}
